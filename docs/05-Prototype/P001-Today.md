...
页面不展示固定日期或星期文案；页面标题与正文属于同一滚动区域，向下浏览时应随内容一起滚动。
右上角使用固定个人头像图标进入个人中心。当天新添加的训练计划置于计划列表顶部。
...
- [ ] 无模板时有清晰入口
- [ ] 可从训练模板选择并添加今日训练计划
- [x] 今日训练计划卡片主体进入本次训练预览/调整页
- [x] 编辑此次训练进入独立 Session 草稿编辑页，不修改原模板
- [x] 本次训练编辑页可修改组数、目标次数、休息时间，并可添加/删除动作
- [x] 今日训练计划开始按钮进入训练页
