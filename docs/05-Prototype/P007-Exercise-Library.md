...
搜索框由外层完整圆角容器承载，输入区域不得以不带圆角的背景覆盖胶囊轮廓。
...
- browse 模式不直接添加
- 选择模式以底部弹层承载，右上角显示关闭按钮；不展示“为训练模板/今日训练选择动作”的说明卡。
- 顶部仅展示搜索输入框；当前版本不展示自定义动作入口，也不得创建本地自定义 Exercise。
- 初次打开默认选中左侧第一个肌群分类（胸）。
- 左侧肌群分类改变主过滤条件；再次选择当前肌群可取消该筛选。
- 切换左侧肌群分类后，右侧动作列表回到顶部。
- 右侧器械分类与肌群分类组合筛选
