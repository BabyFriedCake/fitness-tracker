...
- [x] 正确展示当前动作、组和 Rep 进度。
- [x] 展示当前动作图片或稳定占位。
- [x] 进入训练页后自动启动 draft WorkoutSession。
- [x] 默认 Mock Companion 输入源自动推进 Rep，不需要用户手动计数。
- [x] running 展示暂停、上一动作、下一动作控制。
- [ ] paused 展示全屏暂停状态，继续后恢复原进度。
- [x] 合法 Companion Event 按顺序推进 Runtime。
- [x] 页面不提供手动计数或手动完成组。
- [x] 不展示重复的跳过动作 / 恢复动作入口；切换动作使用上一/下一动作控制。
