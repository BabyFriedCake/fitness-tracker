...
- Today 训练计划、添加计划、本次训练预览/调整、完成禁用流程已对齐。
- 模板列表卡片进入详情页，编辑通过明确入口进入。
- 动作库已对齐左侧肌群、右侧器械筛选和图片卡片布局。
- 历史页已支持月历、月份切换、日期点击和日期训练列表。
- 训练页 running / paused / resting 状态已按 Figma 补齐。
- 训练事实仍通过 Application → Domain → Repository → Database。

Warnings：

- 历史日历肌群标签当前从 `SessionExercise.exerciseNameSnapshot` 派生，
  后续可接入标准 Exercise muscle metadata。
- 已完成代码级和测试级 review，尚未做真实设备截图级视觉验收。
